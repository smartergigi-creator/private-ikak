...
=============================== */
    ...
    function loadBook() {
        ...
        const isMediaViewer = Boolean(flipbook.querySelector(".media-page"));
...
=============================== */
    ...
    function detectPageRatio() {
        ...
        if (mediaPage) {
            const video = mediaPage.querySelector("video");
            if (video?.videoWidth && video?.videoHeight) {
                PAGE_RATIO = video.videoHeight / video.videoWidth;
                return;
            }

            const img = mediaPage.querySelector("img");
...
=============================== */
    ...
    function calculateSize() {
        ...
        const isMediaViewer = viewer?.classList.contains(
        ...
        let w =
            availableW *
            (isMediaViewer ? 0.98 : mobile && landscape ? 0.98 : 0.92);
        let h = w * PAGE_RATIO;
        const maxH =
            availableH *
            (isMediaViewer ? 0.98 : mobile && landscape ? 0.96 : 0.9);
...
=============================== */
    ...
    function initFlipbook() {
        ...
        const isMediaViewer = Boolean(book.querySelector(".media-page"));

        const firstRealPage = book.querySelector(".page img");
        const isLandscapeBook =
            firstRealPage &&
            firstRealPage.naturalWidth > firstRealPage.naturalHeight;

        // Force single page for:
        // media viewer
        // mobile portrait
        // landscape ebooks
        const useSingleMode =
            isMediaViewer || (mobile && !landscape) || isLandscapeBook;
        /* Fake pages */
...
=============================== */
    ...
    // Debounced resize – prevents multiple rapid calls
    ...
    function resizeBook() {
        ...
        resizeTimer = setTimeout(() => {
            // Get current page index before any update
            const currentPage = pageFlip.getCurrentPageIndex();

            // Calculate new size based on current container dimensions
            const newSize = calculateSize();
            const oldSize = pageFlip.getSettings()?.size;
            ...
            // Apply the new size to the flipbook
            pageFlip.update(newSize);

            // Ensure page index is within valid bounds after resize
            const totalPages = pageFlip.getPageCount();
            let clampedPage = Math.min(
                Math.max(currentPage, firstRealPageIndex),
                totalPages - 1,
            );
            if (clampedPage !== pageFlip.getCurrentPageIndex()) {
                pageFlip.turnToPage(clampedPage);
            }

            // Reapply zoom only if it's not the default (1)
            if (zoomLevel !== 1) {
                applyZoom(zoomLevel);
            }

            // Sync UI elements (page numbers, nav buttons)
            if (typeof syncViewerState === "function") syncViewerState();
            if (typeof updateNav === "function") updateNav();
        }, 100); // 100ms debounce – enough for layout to settle
    }
...
=============================== */
    ...
    /* ===============================
   ZOOM WITH CENTER LOCK (NO SHIFT)
=============================== */
    ...
    function applyZoom(v) {
        ...
        // No need to call recenterZoomedBook – the transform origin handles centering.
        // But we must ensure the wrapper is inside a flex‑centered container.
        // Also, after scaling, force a reflow to avoid clipping.
        void wrapper.offsetHeight;
    }

    // Remove recenterZoomedBook entirely – it causes more harm than good.
    // If you still want to keep it for safety, make it a no‑op:
    function recenterZoomedBook() {
        // This function is now empty – scaling with center origin is enough.
        // We keep it to avoid errors in existing calls.
    }
    ...
    function setupZoomControls() {
        ...
        zr.onclick = () => {
            ...
            // No need to call resizeBook – the wrapper scale is now 1, and the book is already at correct size.
        };
...
=============================== */
    /* ===============================
   FULLSCREEN - NO MANUAL RECALCULATION (FIXED)
=============================== */
    function setupFullscreen() {
        ...
        const onFullscreenChange = () => {
            ...
            // Do NOT manually call resizeBook here – let the window resize event handle it.
        };
    ...
    function initFileUpload() {
        ...
        function update(files) {
            ...
            fileList.forEach((f) => {
                ...
                li.textContent = isPdf ? `${f.name} (PDF)` : f.name;
                items.appendChild(li);
            });

            list.style.display = fileList.length ? "block" : "none";
            count.textContent = String(fileList.length);
        }
            ...
            .then(({ ok, data }) => {
                ...
                if (ok && data.status) {
                    ...
                    showMessage(
                        "error",
                        data.message ||
                            "Upload failed. Check file size or format.",
                    );
                }
            })
            .catch((err) => {
                ...
                showMessage("error", err?.message || "Server error");
            });
    });
});
...
function setupFullscreen() {
    ...
    const syncAfterFullscreen = () => {
        ...
        setTimeout(() => {
            if (typeof pageFlip !== "undefined" && pageFlip) {
                // Recalculate and update flipbook size
                const newSize = calculateSize();
                pageFlip.update(newSize);
                // Stay on same page
                const currentIdx = pageFlip.getCurrentPageIndex();
                pageFlip.turnToPage(currentIdx);
