...
public function menus()
{
    ...
    return view('admin.menus', compact('menus', 'parentMenus'));
}
...
public function storeMenu(Request $request)
{
    $validated = $request->validate([
        ...
        'position_type' => 'nullable|in:before,after',
        'reference_menu_id' => 'nullable|exists:menus,id',
        'status' => 'nullable|boolean',
    ]);

    $name = trim((string) $validated['name']);
    $slug = trim((string) ($validated['slug'] ?? ''));
    $slug = $slug !== '' ? Str::slug($slug) : $this->generateUniqueMenuSlug($name);

    $sortOrder = Menu::max('sort_order') + 10;

    if (!empty($validated['reference_menu_id'])) {

        $referenceMenu = Menu::find($validated['reference_menu_id']);

        if ($referenceMenu) {

            if (($validated['position_type'] ?? '') === 'before') {

                Menu::where('sort_order', '>=', $referenceMenu->sort_order)
                    ->increment('sort_order', 10);

                $sortOrder = $referenceMenu->sort_order;
                ...
                $sortOrder = $referenceMenu->sort_order + 10;
            }
        }
    }
    ...
    Menu::create([
        'name' => $name,
        'slug' => $slug,
        'url' => $this->normalizeMenuUrl($validated['url'] ?? ''),
        'parent_id' => $this->normalizeNullableId($validated['parent_id'] ?? null),
...
public function updateMenu(Request $request, $id)
{
    ...
    $validated = $request->validate([
        'name' => 'required|string|max:120',
        'slug' => ['nullable', 'string', 'max:120', Rule::unique('menus', 'slug')->ignore($menu->id)],
        'url' => 'nullable|string|max:255',
        'parent_id' => 'nullable|exists:menus,id',
        'position_type' => 'nullable|in:before,after',
        'reference_menu_id' => 'nullable|exists:menus,id',
    ...
    $parentId = $this->normalizeNullableId($validated['parent_id'] ?? null);
    ...
    if ($parentId !== null && $this->createsMenuParentCycle($menu->id, $parentId)) {
        return back()->withErrors([
            'parent_id' => 'Invalid parent selected. It creates a circular menu hierarchy.'
        ])->withInput();
    }
    ...
    if (
        !empty($validated['reference_menu_id']) &&
        $validated['reference_menu_id'] != $menu->id
    ) {
    ...
    $menu->update([
        ...
        'slug' => $slug !== ''
            ? Str::slug($slug)
            : $this->generateUniqueMenuSlug($name, $menu->id),
        'url' => $this->normalizeMenuUrl($validated['url'] ?? ''),
        'parent_id' => $parentId,
        'sort_order' => $sortOrder,
...
{
    $menu = Menu::findOrFail($id);
    $menu->status = (int) $menu->status === 1 ? 0 : 1;
    $menu->save();

    return back()->with('success', "Menu '{$menu->name}' status updated.");
}
...
public function deleteMenu($id)
{
    ...
    return back()->with('success', "Menu '{$name}' deleted successfully.");
}
...
public function expiredShares(Request $request)
{
    $allowedPerPage = [10, 25, 50, 100];
    $perPage = (int) $request->query('per_page', 10);
    $perPage = in_array($perPage, $allowedPerPage, true) ? $perPage : 10;

    $sort = $request->query('sort', 'expiry_date');
    $direction = strtolower($request->query('direction', 'desc')) === 'asc' ? 'asc' : 'desc';
    $sortableColumns = [
        'file' => 'ebook.title',
        'shared_by' => 'shared_users.name',
        'share_date' => 'ebook.created_at',
        'expiry_date' => 'ebook.share_expires_at',
        'views' => 'ebook.current_views',
    ];
    ...
    $ebooks = Ebook::with(['sharedUser', 'uploader', 'uploadedByUser', 'category'])
        ->leftJoin('users as shared_users', 'ebook.shared_by', '=', 'shared_users.id')
        ->select('ebook.*')
        ->where('ebook.share_enabled', 1)
        ->whereNotNull('ebook.share_expires_at')
        ->where('ebook.share_expires_at', '<', now())
        ->when($search !== '', function ($query) use ($search) {
            $like = '%' . $search . '%';

            $query->where(function ($query) use ($like) {
                $query->where('ebook.title', 'like', $like)
                    ->orWhere('ebook.file_title', 'like', $like)
                    ->orWhere('ebook.slug', 'like', $like)
                    ->orWhere('ebook.share_token', 'like', $like)
                    ->orWhereHas('sharedUser', function ($userQuery) use ($like) {
                        $userQuery->where('name', 'like', $like)
                            ->orWhere('email', 'like', $like);
                    })
                    ->orWhereHas('uploader', function ($userQuery) use ($like) {
                        $userQuery->where('name', 'like', $like)
                            ->orWhere('email', 'like', $like);
                    })
                    ->orWhereHas('uploadedByUser', function ($userQuery) use ($like) {
                        $userQuery->where('name', 'like', $like)
                            ->orWhere('email', 'like', $like);
                    });
            });
    ...
    $totalExpiredShares = Ebook::where('share_enabled', 1)
        ->whereNotNull('share_expires_at')
        ->where('share_expires_at', '<', now())
        ->count();

    return view('admin.expired-shares', compact(
        'ebooks',
        'totalExpiredShares',
        'allowedPerPage',
        'perPage',
...
private function generateUniqueMenuSlug(string $name, ?int $ignoreId = null): string
{
    $baseSlug = Str::slug($name);
    if ($baseSlug === '') {
        $baseSlug = 'menu';
    ...
    while (Menu::when($ignoreId, fn ($q) => $q->where('id', '!=', $ignoreId))
        ->where('slug', $slug)
        ->exists()) {
        $slug = $baseSlug . '-' . $counter;
        $counter++;
...
private function normalizeNullableId($id): ?int
{
    $id = (int) ($id ?? 0);
    return $id > 0 ? $id : null;
}

private function createsMenuParentCycle(int $menuId, int $parentId): bool
{
    $current = Menu::find($parentId);
    while ($current) {
