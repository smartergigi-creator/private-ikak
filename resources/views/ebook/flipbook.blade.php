...
@section('content')
    @php
        ...
        $canShareFile = auth()->check() && ($canShareFile ?? false);
        $isPdf = $ebook->isPdf();
        $isImage = $ebook->isImage();
        $isVideo = $ebook->isVideo();
        $fileUrl = asset(ltrim(str_replace('\\', '/', $ebook->pdf_path), '/'));
        $fileExtension = $ebook->fileExtension() ?: 'file';
        $downloadName = trim($ebook->file_title ?: $ebook->title ?: 'file') . '.' . $fileExtension;
    @endphp
    ...
    <div class="container-fluid vh-100 d-flex flex-column">
        ...
        <div class="row flex-grow-1">
            <div class="col-12 d-flex justify-content-center align-items-center">
                <div id="viewer-wrapper" class="mb-3 {{ $isPdf ? '' : 'media-viewer-wrapper' }}">
                    ...
                    <div class="viewer-toolbar position-absolute top-0 end-0 m-3 d-flex gap-2">
                        <a id="downloadEbook" href="{{ $downloadUrl }}" class="btn btn-light btn-sm"
                            data-download-name="{{ $downloadName }}" aria-label="Download file">
                            <i class="bi bi-download"></i>
                        </a>

                        @if ($canShareFile)
                            <button type="button" class="btn btn-light btn-sm"
                                onclick="openShareModal('{{ $ebook->slug }}')" aria-label="Share file">
                    ...
                    <div id="zoom-wrapper" class="position-relative">
                        <div id="ebook-scale">
                            <div id="flipbook">
                                @unless ($isPdf)
                                    <div class="page media-page" data-page-no="1" data-loaded="1">
                                        @if ($isImage)
                                            <img src="{{ $fileUrl }}" alt="{{ $ebook->title }}" draggable="false">
                                        @elseif ($isVideo)
                                        ...
                                        @else
                                            <div class="media-file-preview">
                                                ...
                                                <span>{{ $ebook->file_title ?: $ebook->title }}</span>
                                            </div>
                                        @endif
                                    </div>
                                @endunless
    ...
    <script>
        (function() {
            ...
            if (!isPdf) {
                ...
                if (loadingText) {
                    loadingText.textContent = 'File loading... 100%';
                }
                return;
            }
